...
Gemini API では，用途やコストに応じて複数のモデルを使い分けることが可能である．以下はモデルの一例である．

*   **`gemini-2.5-pro`**: 性能を最も重視する場合の最上位モデル．複雑な推論や質の高い文章生成に適する．無料枠の場合はトークンの上限に注意．
...
モデルを切り替えるには，`model="..."` の ID を書き換えるだけでよい．上記以外にも多種多様なモデルが使えるので興味があればどんなモデルがあるのか調べてみよう．
